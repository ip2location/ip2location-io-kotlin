...
```{py:function} setApiKey(apiKey)
Set the IP2Location.io API key.
...
```{py:function} DomainWhois(config)
...
```{py:function} toNormalText(domain)
Get Normal Text.
...
```{py:function} toDomainExtension(str)
...
:param integer page: (Optional) Pagination result of the hosted domains. If unspecified, 1st page will be used. 
:::
